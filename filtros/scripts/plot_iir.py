...
# axes[1, 0].plot(x, np.fft.fft(_input))
axes[1, 0].set_xlabel("Frequencia")
axes[1, 0].set_ylabel("Amplitude")

axes[1, 1].magnitude_spectrum(output, Fs=fs)
# axes[1, 1].plot(x, np.fft.fft(output))
axes[1, 1].set_xlabel("Frequencia")
axes[1, 1].set_ylabel("Amplitude")

axes[0, 2].set_title("Resposta ao impulso")
axes[0, 2].plot(resposta)
axes[0, 2].set_xlabel("Taps")
axes[0, 2].set_ylabel("Amplitude")

# axes[1, 2].magnitude_spectrum(resposta, Fs=fs)
axes[1, 2].plot(x, np.fft.fft(resposta))
